...
def process_analysis(machines: List[MachineSchema], data: List[dict]) -> Dict[str, Any]:
...
def calculate_machine_metrics(machine: MachineSchema, entries: List[dict]) -> dict:
...
def time_metrics(entries: List[dict]) -> tuple:
    if not entries:
...
def average_time(times: List[time]) -> time:
    ...
    return time.fromisoformat(f"{int(total//3600//len(times)):02d}:{int((total%3600)//60//len(times)):02d}")

def calculate_consumption(machine: MachineSchema, entries: List[dict]) -> float:
    capacity = machine.battery_size or machine.fuel_tank_size or 0
    prev = None
    total = 0.0
    
    for e in sorted(entries, key=lambda x: x['timestamp']):
        level = e.get('battery_SoC') or e.get('fuel_level')
...
def calculate_efficiency_metrics(metrics: List[dict]) -> dict:
    ...
    return {
        "most_efficient": max(efficiencies, key=lambda x: x[1])[0] if efficiencies else None,
        "least_efficient": min(efficiencies, key=lambda x: x[1])[0] if efficiencies else None,
        "average_hourly": round(sum(x[1] for x in efficiencies)/len(efficiencies), 2) if efficiencies else 0
    }

def calculate_peak_consumption(machines: List[MachineSchema], data: List[dict]) -> dict:
    ...
    for m in machines:
        ...
        for e in sorted(entries, key=lambda x: x['timestamp']):
            level = e.get('battery_SoC') if m.fuel_type == 'electric' else e.get('fuel_level')
